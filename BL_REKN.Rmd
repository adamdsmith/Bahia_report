...
# Bring in detailed habitat raster, "fix" extent, filter to intertidal, ection prior to reprojection as sf object
bl_hab <- raster("Data/lslt_sntl_int_hybrid_final2.img")
bl_hab[bl_hab < 1] <- NA
bl_hab <- trim(bl_hab) %>%
  shift(x = 200, y = 9999900)
recvs_raster <- fasterize(recvs_utm, bl_hab)
bl_hab_lu <- data.frame(hab_code = 1:12,
                        ...
                        habitat_gen = c(rep("mud", 3), rep("mud/sand", 4),
                                        rep("loamy sand", 2), "rock flat with mussels/pools",
                                        "cobble beach", "tidal wetland"),
                        stringsAsFactors = FALSE)
ant_hab <- lapply(seq_len(nrow(recvs_utm)), function(i) {
  ant_i <- fasterize(recvs_utm[i, ], bl_hab) %>% trim() 
  ant_hab <- crop(bl_hab, ant_i) %>% mask(ant_i) %>%
    as.data.frame() %>%
    filter(!is.na(Class_Names)) %>%
    group_by(Class_Names) %>% tally()
  ant_i <- recvs_utm[i, ] %>%
